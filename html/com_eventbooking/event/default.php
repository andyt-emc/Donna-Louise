...
<div id="eb-event-page" class="eb-container eb-category-<?php echo $item->category_id; ?>eb-event<?php if ($item->featured) echo ' eb-featured-event'; ?>" itemscope itemtype="http://schema.org/Event">
	...
	<div class="eb-event-hero hero-banner internal clearfix">
		<?php
			...
			if ($this->config->get('show_image_in_event_detail', 1) && $this->config->display_large_image && $item->image && file_exists(JPATH_ROOT . '/' . $item->image))
			{
				...
				// load FULL image for single event page if available
				$image_path = $item->image;
				$image_path = str_replace('.jpg', '_FULL.jpg', $image_path);

				if (file_exists($image_path)) : ?>
					<img src="<?php echo $baseUri. '/' . $image_path; ?>" class="eb-event-large-image img-polaroid"/>
				<?php else : ?>
			...
			?>
				<a href="<?php echo $largeImageUri; ?>" class="eb-modal"><img src="<?php echo $baseUri . '/media/com_eventbooking/images/thumbs/' . $item->thumb; ?>" class="eb-thumb-left"/></a>
			<?php
			}
	...
	<div id="eb-event-details" class="eb-description">
		...
		<div class="eb-description-details single-col small clearfix" itemprop="description">
			<?php		
...
<div id="event-below" class="clearfix">
	<?php
		/*
		 * Custom code
		 *
		 * load custom event module position
		 */ 	
		jimport('joomla.application.module.helper');
	    $modules = JModuleHelper::getModules('event-single'); 
